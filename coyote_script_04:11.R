...
#            alternation limit reached - FIXED BY ADDING MAXIT = 1000
...
project_data$pred_coyote_nb <- predict(H3_nb, type = "response") 

# remove outlier (what's going on with that?)
project_data_no_outlier <- project_data %>%
  filter(pred_coyote_nb <= 200)

## infrastructure line ----

inf_line_plot_NB <- ggplot(project_data_no_outlier, aes(x = infrastructure_line, y = pred_coyote_nb)) +
  geom_point() +  # Use geom_point to plot points
  geom_smooth(method = "glm") + 
  labs(x = "Infrastructure line",
       y = "Prediced coyote count") +
  theme(legend.position = "NONE",
        panel.background = element_blank(),
        panel.border = element_rect(fill = NA),
        axis.text.x = element_text(size = 15),
...
gravel_road_plot_NB <- ggplot(project_data_no_outlier, aes(x = gravel_road, y = pred_coyote_nb)) +
  geom_point() +  # Use geom_point to plot points
  geom_smooth(method = "glm") +  # Add smoothed line based on the GLM
  labs(x = "Gravel Road",
       y = "Prediced coyote count") +
...
deer_plot_NB <- ggplot(project_data_no_outlier, aes(x = deer_tot_det, y = pred_coyote_nb)) +
  geom_point() +  # Use geom_point to plot points
  geom_smooth(method = "glm") +  # Add smoothed line based on the GLM
  labs(x = "deer",
       y = "Prediced coyote count") +
...
hare_plot_NB <- ggplot(project_data_no_outlier, aes(x = hare_tot_det, y = pred_coyote_nb)) +
  geom_point() +  # Use geom_point to plot points
  geom_smooth(method = "glm") +  # Add smoothed line based on the GLM
  labs(x = "Hare",
       y = "Prediced coyote count") +
...
moose_plot_NB <- ggplot(project_data_no_outlier, aes(x = moose_tot_det, y = pred_coyote_nb)) +
  geom_point() +  # Use geom_point to plot points
  geom_smooth(method = "glm") +  # Add smoothed line based on the GLM
  labs(x = "moose",
       y = "Prediced coyote count") +
...
NEG_BINOM_figure <- ggarrange(inf_line_plot_NB, gravel_road_plot_NB, deer_plot_NB, hare_plot_NB, moose_plot_NB)
NEG_BINOM_figure
