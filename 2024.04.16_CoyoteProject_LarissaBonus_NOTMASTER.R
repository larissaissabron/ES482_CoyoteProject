...
# Moose detections
# Histogram of number of cameras that have each value of total detections of moose
project_data %>% 
  group_by(moose_tot_det) %>% 
  summarise(n = n()) %>% 
  ggplot(aes(x=n)) +
  geom_histogram()

# Where are the highest detections?
project_data %>% 
  filter(moose_tot_det > 15) %>% 
  relocate(moose_tot_det) %>% 
  arrange(desc(moose_tot_det))

# Outliers in species detections

# Boxplot
project_data %>%
  pivot_longer(cols = c(moose_tot_det, hare_tot_det, coy_tot_det, wolf_tot_det),
               names_to = "tot_species",
               values_to = "tot_det") %>% 
  ggplot(aes(x = tot_species, y = tot_det)) +
  geom_boxplot() +
  labs(
    x = "Species",  
    y = "Total Detections", 
    title = "Boxplot of Total Species Detection",
 ) +
  scale_x_discrete(labels = c("Moose", "Snowshoe Hare", "Coyote", "Grey Wolf"))

# Outliers in landscape cover
project_data %>%
  pivot_longer(cols = c(infrastructure_line, gravel_road, trail, seismic_line, road_unimproved),
               names_to = "linear_feats",
               values_to = "percent_cover") %>% 
  ggplot(aes(x = linear_feats, y = percent_cover)) +
  geom_boxplot() +
  labs(
    x = "Linear Feature",  
    y = "Percent Cover (within 1 km of cameras)", 
    title = "Boxplot of Percent Cover of Linear Features",
  ) +
  scale_x_discrete(labels = c("Infrastructure Line", "Gravel Road", "Trails", "Seismic Line", "Unimproved Road"))

# What is the distribution of variables like at a landscape scale?
project_data %>% 
  pivot_longer(cols = c(infrastructure_line, gravel_road, trail, seismic_line, road_unimproved, forest, grass, shrub, water),
               names_to = "land_cover",
               values_to = "percent_cover") %>% 
  ggplot(aes(x = array, y = percent_cover, color = percent_cover)) +
  geom_point() +
  facet_wrap(~land_cover, scales = "free_y") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))


# Heat map
project_data_long <- project_data %>% 
  pivot_longer(cols = c(infrastructure_line, gravel_road, trail, seismic_line, road_unimproved, forest, grass, shrub, water),
               names_to = "land_cover",
               values_to = "percent_cover") %>% 
  group_by(array, land_cover) %>%
  mutate(mean_percent_cover = mean(percent_cover)) %>%
  ungroup()

# Matrix heatmap
project_data_long %>% 
  na.omit() %>% 
  mutate(percent_cover = ifelse(percent_cover == 0, 0.0000001, percent_cover)) %>% 
  ggplot(aes(x = array, y = land_cover, fill = percent_cover)) +
  geom_tile(color = "black") +
  geom_text(aes(label = round(mean_percent_cover, 3)), color = "black", size = 3) + 
  scale_fill_gradient(low = "honeydew", high = "grey29", trans = "log") +  
  labs(x = "Landscape Unit", y = "Landscape Feature", title = "Percent Coverage of Landscape Features") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "none") +
  scale_y_discrete(labels = c("forest" = "Forest", "grass" = "Grass", "gravel_road" = "Gravel\nRoad", "infrastructure_line" = "Infrastructure\nLine", "road_unimproved" = "Road\nUnimproved", "seismic_line" = "Seismic\nLine", "shrub" = "Shrub", "trail" = "Trail", "water" = "Water")) 

# Amount of co-occurence of different competitors
...
# get together everything potentially of interest into one file  --------
project_data2 <- covariates %>% 
  ...
  # Further down the line you are going to appreciate removing rows with NA value
  na.omit() %>% 
  dplyr::select(site, array, camera, coyote, black_bear, fisher, cougar, lynx, grey_wolf, wolverine)

# Graph it
# Correlation matrix
chart.Correlation(project_data2[c("coyote", "black_bear", "fisher", "cougar", "lynx", "grey_wolf", "wolverine")],
                  histogram = TRUE, 
                  method = "spearman",
                  text.scale = 6,
                  main="Correlation Matrix for Potential Competitor Mammals")

title("Correlation Matrix for Potential Competitors", line = 3)
